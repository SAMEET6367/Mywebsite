    ...
    <header className="sticky top-0 z-50 border-b border-ink-700/70 bg-ink-950/80 backdrop-blur-md">
      <Container className="flex h-16 items-center justify-between">
        ...
        <button
          ...
          aria-expanded={open}
          className="flex h-10 w-10 items-center justify-center rounded-full border border-ink-700 text-paper md:hidden"
        >
          ...
          <Container className="flex flex-col gap-1 py-4">
            {LINKS.map((link) => (
              <a
                ...
                className="rounded-lg px-2 py-3 text-base text-fog transition-colors hover:bg-ink-900 hover:text-paper"
              >
                {link.label}
              </a>
            ))}
            ...
            <a
              href="#contact"
              onClick={() => setOpen(false)}
              className="mt-2 rounded-full bg-gradient-to-r from-violet to-cyan px-4 py-3 text-center text-sm font-medium text-ink-950"
            >
              Let&apos;s talk
            </a>
          </Container>
